...
//     try {
//       const user_id = await getCurrentUserId();
//       const response = await fetch(`http://192.168.1.9:3000/income/${user_id}`);
...
//         ? `http://192.168.1.9:3000/api/balances/user/${user_id}`
//         : 'http://192.168.1.9:3000/api/balances';
...
//   const parsedIncome = parseFloat(inputIncomeValue); // استخدام inputIncomeValue هنا
//   if (isNaN(parsedIncome)) return;

//   try {
//     const user_id = await getCurrentUserId();
//     // إذا كانت قيمة الدخل فارغة (null أو غير موجودة)، استخدم POST
//     const url = income === null || income === undefined
//       ? `http://192.168.1.9:3000/income/${user_id}` // POST لخلق دخل جديد
//       : `http://192.168.1.9:3000/income/${user_id}`; // PUT لتحديث الدخل الحالي
//     const method = income === null || income === undefined ? 'POST' : 'PUT';
...
//       body: JSON.stringify({
//         user_id,
//         income: parsedIncome, // إرسال قيمة الدخل الجديدة
//       }),
//     });
...
//     const data = await response.json();
//     setIncome(data.income || parsedIncome);  // تحديث حالة الدخل بالقيمة الجديدة
//     setTotalBalance(data.balance_limit || 0); // تحديث الرصيد إذا رغبت في ذلك
//     setExpense(data.expense || 0); // تحديث المصاريف إذا رغبت في ذلك
//     setInputIncomeValue(''); // إعادة تعيين حقل الإدخال بعد الحفظ

//   } catch (error) {
//     console.error('Error saving income:', error);
//   } finally {
//     setModalIncomeVisible(false); // إغلاق المودال بعد العملية
//   }
// };
...
//         </View>
//       </Modal>
//       <Modal animationType="slide" transparent={true} visible={modalIncomeVisible}>
//         <View style={styles.modalOverlay}>
//           <View style={styles.modalContainer}>
  ...
  const handleSaveIncome = async () => {
    ...
    try {
      ...
      const url = income == null
        ? `http://192.168.1.9:3000/income/${user_id}`
        : `http://192.168.1.9:3000/income/${user_id}`;
