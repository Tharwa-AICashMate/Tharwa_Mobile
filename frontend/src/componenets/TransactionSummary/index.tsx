...
const TransactionSummary: React.FC<TransactionSummaryProps> = ({
  ...
  const fetchIncome = async () => {
    try {
      const user_id = await getCurrentUserId();
      const response = await fetch(`http://192.168.1.5:3000/income/${user_id}`);
  ...
  const handleSaveBalance = async () => {
    ...
    try {
      ...
      const url = totalBalance
        ? `http://192.168.1.5:3000/api/balances/user/${user_id}`
        : 'http://192.168.1.5:3000/api/balances';
  ...
  const handleSaveIncome = async () => {
    ...
    try {
      ...
      const url = income == null
        ? `http://192.168.1.5:3000/income/${user_id}`
        : `http://192.168.1.5:3000/income/${user_id}`;
